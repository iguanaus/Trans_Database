...
class Restaurant:
    ...
    def get_menu_link_from_google(self, logger):
        ...
        logger.log(msg='Trying to get menu from google for {0}'.format(self.name), level=logging.INFO)
    ...
    def find_menu_link_from_postmates(self, logger, location):
        with requests.Session() as session:
            if location == 'Boston, MA':
                r = session.get('https://order.postmates.com/v1/place_search?lat=42.360406000000005&lng=-71.05799299999998&q={0}'.format(self.name))
                ...
                for item in soup.find_all('div', class_='catalog-product'):
                    mi = MenuItem(dish_name=item.find('div', class_='title').text.strip(),
                                  dish_size=None,
                                  dish_price=item.find('div', class_='price').text.strip(),
                                  dish_cals=None,
                                  dish_items=None)
                    self.menu.append(mi)
